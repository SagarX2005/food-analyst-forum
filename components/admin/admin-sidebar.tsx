...
export function AdminSidebar({ isMobileOpen, onMobileClose }: AdminSidebarProps) {
  ...
  const NavItem = ({
    ...
    link: { name: string; href: string; icon: React.ElementType; exact?: boolean };
  }) => {
      ...
      <Link href={link.href} onClick={() => onMobileClose()} className="block">
        ...
        >
          <div className="flex items-center gap-3">
            <Icon
              className={`h-4 w-4 ${isActive ? "text-[#4a9d23]" : "text-slate-400 group-hover:text-slate-600"}`}
            />
            {link.name}
          </div>
    ...
    <div className="flex h-full flex-col gap-6 p-4">
      <div className="flex items-center gap-2 pl-2">
        ...
        <div className="flex flex-col">
          <span className="text-sm leading-tight font-extrabold tracking-tight text-[#0a2a4a]">
      ...
      <div className="scrollbar-hide flex-1 space-y-6 overflow-y-auto">
        ...
        <div>
          <h4 className="mb-2 px-3 text-xs font-extrabold tracking-widest text-slate-400 uppercase">
            System
          </h4>
          <div className="space-y-1">
            {SYSTEM_LINKS.map((l) => (
        ...
        {superAdmin && (
          <div>
            <h4 className="mb-2 px-3 text-xs font-extrabold tracking-widest text-[#4a9d23] uppercase">
              Governance
            </h4>
            <div className="space-y-1">
      ...
      <div className="mt-auto border-t border-slate-200 pt-4">
        <Link href="/">
          <Button
            variant="outline"
            className="h-9 w-full justify-center text-xs font-bold text-slate-600"
          >
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 border-r border-slate-200 bg-white transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 lg:shadow-sm ${isMobileOpen ? "translate-x-0 shadow-xl" : "-translate-x-full"} `}
      >
        {/* Mobile Close Button */}
        <button
          onClick={onMobileClose}
          className="absolute top-4 right-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 lg:hidden"
        >
