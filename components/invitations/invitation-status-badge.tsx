...
const ACCESS_REQUEST_CONFIG: Record<
  AccessRequestStatus,
...
> = {
  ...
  invitation_sent: { label: "Invite Sent",     className: "bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400" },
  accepted:        { label: "Accepted",        className: "bg-[#0a2a4a]/10 text-[#0a2a4a] dark:bg-[#0a2a4a]/40 dark:text-blue-200" },
...
export function AccessRequestStatusBadge({ status, className }: AccessRequestStatusBadgeProps) {
  ...
  return (
    <Badge
      variant="outline"
      className={cn("border-transparent text-xs font-semibold uppercase tracking-wide", cfg.className, className)}
    >
      {cfg.label}
    </Badge>
  );
