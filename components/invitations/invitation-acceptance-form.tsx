...
export function InvitationAcceptanceForm({
    ...
    return (
      <div className="flex flex-col items-center text-center space-y-5 py-8">
        ...
        <div className="space-y-2">
          <h3 className="text-xl font-extrabold text-[#0a2a4a] dark:text-foreground">
            Welcome to FAF!
          </h3>
  ...
  return (
    <div className="space-y-5">
      {/* Invitation summary */}
      <div className="rounded-xl border border-[#4a9d23]/30 bg-[#4a9d23]/5 px-4 py-3 space-y-2">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Your Invitation</p>
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
          <span className="text-muted-foreground">Email:</span>
          <span className="font-medium text-foreground">{email}</span>
          <span className="text-muted-foreground">Assigned Role:</span>
          <span>
            <Badge variant="green" className="text-[10px]">{assignedRole}</Badge>
          </span>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        {serverError && (
          <div className="rounded-xl border border-destructive/40 bg-destructive/8 px-4 py-3 text-xs font-medium text-destructive">
            {serverError}
          </div>
        )}
        ...
        <p className="text-xs text-muted-foreground">
          Your assigned role (<strong>{assignedRole}</strong>) is pre-set from the invitation
          and cannot be changed during account setup.
        </p>
